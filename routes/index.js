...
router.get('/temps/:file', function(req, res, next) {
    return res.sendFile(path.resolve(path.join('temps', req.params.file)));
});
...
router.post('/notes/summarize', requiresLogin, enableSmmryCors, function (req, res, next) {
    ...
    if (req.body.id) {
        Note.findById(req.body.id, function(err, note) {
            ...
            fs.writeFile('../temps/' + req.body.id, text, function(err) {
                if(err) return res.send({ ok: false, err: err });
                var url = 'http://api.smmry.com/&SM_API_KEY=944CDDA147&SM_LENGTH=5&SM_URL=http://nod.chimerical.ca/temps/' + req.body.id;
